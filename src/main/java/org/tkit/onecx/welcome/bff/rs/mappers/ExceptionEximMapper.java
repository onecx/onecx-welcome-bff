...
import gen.org.tkit.onecx.welcome.exim.client.model.EximProblemDetailResponse;

@Mapper(uses = { OffsetDateTimeMapper.class })
public interface ExceptionEximMapper {
        ...
        if (ex.getResponse().getStatus() == 500) {
            ...
            if (ex.getResponse().getMediaType() != null
                ...
                return Response.status(ex.getResponse().getStatus())
                        .entity(map(ex.getResponse().readEntity(EximProblemDetailResponse.class))).build();
            } else {
    ...
    ProblemDetailResponseDTO map(EximProblemDetailResponse problemDetailResponse);
